...
Этап "по желанию" из [plan-2026-07-10](plan-2026-07-10-autonomous-agents.md) (строка 194). Высокоуровневый план и ключевые решения приняты 2026-07-14; реализация отложена (не активный этап). Документ - чтобы решения не потерялись.

## Решение по каналу: вариант D - локальная морда прямо на llm

llm имеет **прямой публичный IP, без NAT**. Панель живёт целиком на llm, **ни от кого не зависит**: без Cloudflare Tunnel, без reverse-tunnel на внешний шаред-хост.

Отвергнутые варианты (судейская панель + разбор прецедента внешнего шаред-хоста):
- **Cloudflare Tunnel + Access** - изолирует поверхность, но зависимость от CF как канала И identity. Выбрана независимость.
- **Reverse/forward-tunnel через внешний шаред-хост** (`ssh -L` -> nginx+LE+basic) - переиспользует готовое, но тащит чужой multi-tenant хост в цепь доверия control-plane; для управления парком неприемлемо.
...
Отвергнутые:
- **mTLS (клиентский серт)** - строго сильнее по поверхности (без серта TLS-handshake отклоняется, порт невидим), но выбрано против из-за UX установки серта на телефон.
- **Basic Auth** - слабее, один фактор.
...
- **Не хранит LUKS-пассфразу** - разлочка /data остаётся ручной по ssh.
- **Не заводит БД** - состояние это файлы реестра.

## Связано

- Прецедент паттерна ssh-tunnel+панель: внутренний сервис на внешнем шаред-хосте (forward `-L` -> nginx+LE+basic-auth).
