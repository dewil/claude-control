...
llm имеет **прямой публичный IP `161.104.35.228`, NAT нет** (см. memory `project_llm_vm_selectel`). Панель живёт целиком на llm, **ни от кого не зависит**: без Cloudflare Tunnel, без reverse-tunnel на s79.

Отвергнутые варианты (судейская панель + разбор прецедента s79):
- **Cloudflare Tunnel + Access** - изолирует поверхность, но зависимость от CF как канала И identity. dwl выбрал независимость.
- **s79 reverse/forward-tunnel** (паттерн bot-sm2-adm: `ssh -L` на alp2 -> nginx+LE+basic) - переиспользует готовое, но тащит shared-s79 (50+ сайтов) в цепь доверия control-plane; для управления парком неприемлемо.
...
- **Basic Auth** (как s79) - слабее, один фактор.
...
- Прецедент паттерна ssh-tunnel+панель: s79 `bot-sm2-adm.alp.dewil.ru` (forward `-L` на alp2, nginx+LE+basic-auth).
