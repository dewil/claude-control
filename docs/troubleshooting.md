...
Это нормальное поведение. Сессии, которые поднимает `claude-rc`, гаснут на простое - если к ним никто не подключается, процесс `claude remote-control` сам выходит спустя какое-то время (порядка часов, точный таймаут не документирован). Юнит `ccsession-<uuid>` транзиентный, поэтому вместе с процессом уходит и он, а сессия пропадает из приложения Claude.

**Что делать:** поднимай сессию непосредственно перед тем, как она понадобится, а не заранее. Если успела погаснуть - подними заново: транскрипт никуда не делся, `claude-rc sessions <имя>` покажет ее в меню, `claude-rc up <имя> <uuid>` вернет в строй. Что сейчас поднято - `claude-rc live`.
...
> сессии поднимаются тапом в боте, `claude-control-watchdog` и его таймер отключены и
> установщиком не ставятся. Оставлено на случай отката. Сегодняшний аналог "юнит жив,
> а сессия недостижима" - `claude-rc reap` (гасит только под `CLAUDE_RC_REAP_ARM=1`).
...
Сессий в tmux больше нет: `claude-rc` поднимает их юнитами `ccsession-<uuid>`. Если такое видно - это слой АГЕНТОВ: `claude-agent-session` держит tmux-клиент на своем сокете `agent-<name>.g<gen>` (то есть `tmux -L <сокет>`, а не сервер по умолчанию). Смотреть надо туда: `claude-rc agent status <name>`, `claude-rc agent attach <name>`. Ошибка от голого `tmux ls` про эти сессии ничего не говорит - у них другой сокет.
