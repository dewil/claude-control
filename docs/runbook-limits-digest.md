...
превращается в уведомление. Окно узнается по близости `resets_at` с допуском
`WINDOW_MATCH_S`, а НЕ по равенству строки: провайдер считает его как "сейчас +
остаток", момент дрейфует на секунду-другую на каждом опросе, и сравнение строк
...
Память хоста рисуется тем же градусником, что и лимиты: заливка - ЗАНЯТОЕ (`total - MemAvailable`), тренд развернут по смыслу (свободное падает - занятое растет). Без известного объема машины остается прежняя строка цифрами.
