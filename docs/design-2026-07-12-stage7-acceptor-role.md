...
## Дополнение 2026-07-18: quote-gate + zero-findings (роль rev 2)

Пост-GO усиление вердикта по итогам разбора практики plan-review скиллов
(gstack, MIT). Два механических гейта в парсере воркера (`extract_verdict`),
активны только для снапшотов роли `role_rev >= 2` (старые замороженные
снапшоты сохраняют прежнюю семантику - демоция по отсутствующим полям была
бы ложной):

- **Quote-gate (reject).** Каждый finding несёт `quote` - буквальную выдержку
  из диффа. `reject` без хотя бы одной цитаты, подтверждённой подстрочным
  матчем по диффу (как есть или без ведущих `+`/`-`/пробела; тривиально
  короткие < 6 символов не считаются), демотируется в `uncertain`
  (needs-human). Бьёт по главному источнику false-reject: судья уверенно
  описывает дефект, которого в артефакте нет. Проверка по ПОЛНОМУ списку
  findings до усечения (как blocker-инвариант). Известный край: reject
  пустого/косметического диффа, где цитировать нечего, уйдёт в uncertain -
  человеку; направление отказа безопасное.
- **Zero-findings (accept).** Обязательный `checks`: по строке на каждый
  критерий миссии с явным статусом. Молчание по критерию = проверка не
  выполнена: `accept` без непустого `checks` демотируется в `uncertain`.
  Закрывает неразличимость "accept после полной проверки" и "accept, где
  судья молча пропустил измерение".

Реализация: `bin/claude-agent-review` (`extract_verdict(text, diff, strict)` +
`_manifest_role_rev`, strict от rev замороженного манифеста агента),
`roles/acceptor` rev 2 (prompt + manifest sha256). `quote` эмитится в
findings результата (needs-human видит цитату). Тесты: parser 36 (18 новых:
оба гейта, галлюцинированная/короткая цитата, цитата за усечением, legacy)
+ strict-wiring 2 (дифференциальная пара rev2/rev1 сквозь main). Проверка
эффекта на confusion matrix ideal-корпуса (§corpus) - отдельным живым
прогоном (стоимость - решение оператора).
