...
Запуск: python3 tests/test-voice-report.py
...
_voice_report_path = str(BIN / "voice-report")
# spec_from_file_location без явного loader'а не находит его для файла без
# расширения .py (см. tests/test-agent-task-lifecycle.sh, тот же прием).
_loader = SourceFileLoader("voice_report", _voice_report_path)
spec = importlib.util.spec_from_file_location("voice_report", _voice_report_path, loader=_loader)
